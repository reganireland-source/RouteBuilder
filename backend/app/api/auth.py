# ─────────────────────────────────────────────────────────────────────────────
# auth.py — authentication check endpoints, for ALL THREE auth models.
#
# Route prefix: /api/auth  (this router has prefix="/auth"; main.py mounts it
# under "/api", so every path below is /api/auth/...).
#
# This app supports three auth models, selected by the AUTH_MODE environment
# variable (see app/auth/oidc.py's auth_mode() and app/main.py's auth_guard,
# which is what actually enforces any of them — nothing in this router
...
#
#   entra — the Microsoft Entra ID (Azure AD) equivalent: every request needs
#   a valid Entra-issued bearer token; write access additionally requires the
#   ENTRA_ADMIN_ROLE App Role. See docs/entra-setup.md and app/auth/entra.py.
#
# Endpoints:
...
#                            which answers the same question for ALL modes.
#   GET  /api/auth/config  — which mode is active, and (in okta/entra mode)
#                            the public OIDC values — issuer, client id — the
#                            frontend needs to redirect the browser to the
#                            provider's login page. Called before the browser
#                            has ever signed in, so it must stay reachable
#                            with no token at all in every mode (see
#                            _SSO_PUBLIC_PATHS in app/main.py).
# ─────────────────────────────────────────────────────────────────────────────
import os
import secrets
...
from ..auth.oidc import auth_mode
...
def auth_config():
    ...
    whichever mode is active. The one thing the frontend calls before it
    knows anything else about itself — including before it has ever signed
    in — so it stays reachable with no token in every mode (see
    _SSO_PUBLIC_PATHS in app/main.py; admin-key mode never required a token
    for GETs anyway).
    ...
    with the okta/entra responses below.
    ...
    rather than attempting a redirect to nowhere.

    Response, entra mode: {"mode": "entra", "entra": {"issuer", "client_id"} |
    null}, same null-when-unconfigured shape as okta above.

    issuer/client_id are NOT secrets in either case: they are exactly what
    the provider's own hosted login page needs from the browser to start a
    PKCE flow, and a public OIDC client (a single-page app) never holds a
    client_secret at all.

    Auth: public read endpoint; no token required, in any mode.
    """
    mode = auth_mode()
    if mode == "okta":
    ...
    if mode == "entra":
        ...
        entra = {"issuer": settings.issuer, "client_id": settings.client_id} if settings else None
        return {"mode": "entra", "entra": entra}
