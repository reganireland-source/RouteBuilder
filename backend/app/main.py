...
#
#   request  ─→ 1. request_context      (correlation id, access log, timing)
#            ─→ 2. security_headers     (CSP + hardening headers on responses)
#            ─→ 3. BodySizeLimit        (413 on oversized bodies)
#            ─→ 4. admin_write_guard    (403/503 auth + 429 rate limit)
#            ─→ 5. CORSMiddleware       (preflight + CORS response headers)
#            ─→ router / endpoint
#   response ←─ unwinds in the reverse order.
#
# Why this order:
#   • request_context outermost  → every response, including the 413/503/403
#     short-circuits below it, gets an X-Request-ID and one access-log line.
#   • security_headers next      → CSP and friends are attached to those
#     short-circuit responses too, not just to endpoint responses.
#   • body cap before auth       → an oversized body is dropped as cheaply as
#     possible, before any token comparison or handler work.
#   • CORS innermost             → unchanged from the original arrangement; it
#     still answers preflights before the router is reached. Note the trade-off:
#     rejections produced by the outer layers do not carry CORS headers, so a
#     browser sees them as network errors rather than readable 4xx bodies.
# ══════════════════════════════════════════════════════════════════════════════
