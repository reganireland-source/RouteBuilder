/**
 * ============================================================================
 *  generateDiagram.ts — export a set of routes as a Straight Line Diagram (SLD).
 * ============================================================================
 *
 * An SLD (Straight Line Diagram) is the schematic a telco hands a customer: it
 * lays every hop of a circuit out along ONE horizontal line — customer site,
 * then each node (CLS / PoP / branching unit), then customer site — with the
 * cable systems, distances and latencies labelled above/below. It is NOT a
 * geographic map; the line is straightened out for clarity.
 *
 * This module produces the same SLD in three document formats. Each is a
 * standalone PUBLIC export that triggers a browser download:
 *
 *   • generateStraightLineDiagram(pins, nodes, version?)
 *       → a multi-page PDF (via jsPDF) for ad-hoc pinned routes: a cover page
 *         listing the circuits, then one landscape A4 page per route.
 *
 *   • generateSldFromProject(project, pins, nodes, version?)
 *       → the same PDF but for a saved Project: it pulls A-End/Z-End customer
 *         details, service specs and worker/protect pairs from the project's
 *         circuits, deduplicates worker+protect pins to one page per circuit,
 *         and fills project metadata into the cover + footer.
 *
 *   • generateDrawioXml(pins, nodes, project?): string
 *       → returns editable draw.io / diagrams.net XML (mxGraph). The caller
 *         wraps it in a .drawio download. One "page" per circuit, offset in X.
 *
 *   • generateVisioVsdx(pins, nodes, project?): Promise<void>
 *       → builds a Microsoft Visio .vsdx (an OPC ZIP of Visio XML parts, via
 *         JSZip) and downloads it. One Visio page per circuit.
 *
 * SHARED DRAWING MODEL
 * --------------------
 * All three formats share the same idea, implemented per-format:
 *   1. buildRouteView() flattens a route into { nodes, segments, totalKm,
 *      totalLatency, systems } — the minimal data the drawings need.
 *   2. Node X-positions are laid out PROPORTIONALLY to cumulative segment
 *      length along the line, so the diagram roughly reflects real distances.
 *   3. First/last nodes are drawn inside dashed "customer site" boxes; inner
 *      nodes get an icon by type (POP / CLS / MMR) and a label.
 *   4. Protected (worker+protect) circuits are drawn as a diamond/lens: the two
 *      shared endpoints in the middle, worker path on top, protect below.
 *
 * The many lower-case helpers (drawDiagram, drawPanels, drawFooter, drawCover,
 * attrRow, drawNodeIcon, …) are the jsPDF drawing primitives for the PDF path;
 * all measurements are in millimetres on a landscape A4 page (see the layout
 * landmark constants below).
 *
 * Glossary: SLD = Straight Line Diagram · CLS = Cable Landing Station ·
 *           MMR = Meet-Me Room · RTD = Round-Trip Delay (= 2 × one-way latency).
 * ============================================================================
 */
...
/**
 * PUBLIC EXPORT. Generate and download a multi-page SLD **PDF** for a set of
 * ad-hoc pinned routes (no project context). Produces a cover page (circuit
 * index table) followed by one landscape-A4 page per pinned route, then calls
 * doc.save() to download `SLD-<date>[-<version>].pdf`. No-op if no pins.
 *
 * @param pinnedRoutes routes to document (each becomes one page)
 * @param nodes        full node list, used to resolve ids → names/countries/icons
 * @param version      optional label (e.g. "Draft"/"Final") stamped on cover+footer
 */
...
/**
 * PUBLIC EXPORT. Generate and download an SLD **PDF for a saved Project**. Same
 * page layout as generateStraightLineDiagram, but richer: worker+protect pins
 * that share a circuitId are collapsed to one page each (using the worker pin +
 * the circuit's protect_route_snapshot to draw the diamond), and the circuit's
 * A-End/Z-End + service details and the project metadata fill the panels,
 * cover and footer. Downloads `SLD-<projectName>-<date>[-<version>].pdf`.
 */
...
/**
 * PUBLIC EXPORT. Build **editable draw.io / diagrams.net XML** (mxGraph model)
 * for the given routes. Unlike the PDF this RETURNS a string (the caller wraps
 * it into a .drawio file download); it does not touch the DOM. Each circuit is
 * laid out on its own horizontal band, offset by PAGE_DX in X, with title box,
 * cable polyline, segment labels, per-node shapes, A/Z-End info boxes and a
 * small service table. `project` (optional) supplies circuit service details.
 *
 * @returns a complete `<mxfile>…</mxfile>` XML document as a string.
 */
...
/** Escape a string for safe inclusion in Visio's XML. */
function escVml(s: string): string {
  return s.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;')
}

/**
 * PUBLIC EXPORT. Build and download a **Microsoft Visio `.vsdx`** of the SLD.
 * A .vsdx is an OPC package (a ZIP of XML parts): this assembles the required
 * `[Content_Types].xml`, `_rels`, `visio/document.xml` and one
 * `visio/pages/pageN.xml` per circuit (each holding Visio <Shape> geometry in
 * inches — title banner, cable polyline, node boxes, A/Z-End boxes, service
 * table), zips them with JSZip, and triggers a `SLD-<date>.vsdx` download.
 * Async because JSZip.generateAsync returns a Promise.
 */
